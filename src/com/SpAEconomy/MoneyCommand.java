...
public class MoneyCommand implements CommandExecutor {
		...
		if (args.length == 0) {
			if (sender instanceof Player) {
				if (sender.hasPermission("spaeconomy.money")) {
					displayMoney(sender, sender.getName());
					return true;
				}
			} else {
			 ...
			 */if (args[0].equalsIgnoreCase("hide") && sender.hasPermission("spaeconomy.hide")) {
				if (sender instanceof Player) {
					if (!main.accounts.exists(sender.getName(), false)) {
						sender.sendMessage(ChatColor.RED + "You don't have a bank account!");
					} else if (main.accounts.getHidden(sender.getName())) {
						main.accounts.setHidden(sender.getName(), false);
						sender.sendMessage(ChatColor.DARK_GREEN + "Showing you to the world!");
					} else {
						main.accounts.setHidden(sender.getName(), true);
			 ...
			 */if (args[0].equalsIgnoreCase("hide") && sender.hasPermission("spaeconomy.hide")) {
				if (!main.accounts.exists(sender.getName(), false)) {
					sender.sendMessage(ChatColor.RED + "You don't have a bank account!");
				} else if (main.accounts.getHidden(args[1])) {
					main.accounts.setHidden(args[1], false);
					sender.sendMessage(ChatColor.DARK_GREEN + "Unhiding " + args[1]);
				} else {
					main.accounts.setHidden(args[1], true);
			...
			if (args[0].equalsIgnoreCase("pay") && sender.hasPermission("spaeconomy.pay")) {
				...
				if (main.accounts.getBalance(sender.getName()) - amount < 0) {
				...
				main.accounts.takeMoney(sender.getName(), amount);
				main.accounts.giveMoney(args[1], amount);

				main.getServer().getPlayerExact(args[1]).sendMessage(ChatColor.DARK_GREEN + sender.getName() + " payed you " + amount + " " + SpAEconomy.moneyName);
				...
				main.accounts.takeMoney(args[1], amount);
				...
				main.accounts.setBalance(args[1], balance);
	...
	private void displayMoney(CommandSender sender, String playerAccount) {
		if (!main.accounts.exists(playerAccount, false)) {
			sender.sendMessage(ChatColor.RED + playerAccount + " has no bank account or has no money.");
		} else {
			sender.sendMessage(ChatColor.DARK_GREEN + playerAccount + " has " + SpAEconomy.roundToDecimals(main.accounts.getBalance(playerAccount), 2) + " " + SpAEconomy.moneyName);
		}
	}
}